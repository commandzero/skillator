...
#[test]
fn target_save_declares_clone_local_target_state_in_root_ignore() {
    let fixture = Fixture::new();
    let session = TargetWorkflow::load(&fixture.target).unwrap();
    let prepared =
        TargetWorkflow::prepare_save(&fixture.paths, &session, session.config.clone()).unwrap();
    ...
    assert_eq!(
        std::fs::read_to_string(fixture.target.join(".gitignore")).unwrap(),
        "/.agents/skillator.yaml\n/.agents/.gitignore\n"
    );
    assert_eq!(
...
fn first_target_save_creates_required_paths_without_initialization_diagnostics() {
    ...
    assert_eq!(
        std::fs::read_to_string(target_root.join(".gitignore")).unwrap(),
        "/.agents/skillator.yaml\n/.agents/.gitignore\n"
    );
}
...
impl WorktreeFixture {
    fn new() -> Self {
        ...
        support::git(&primary, &["commit", "-m", "root ignore contract"]);

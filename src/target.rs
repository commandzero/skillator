...
pub fn observe(
    ...
    for directory in config.skill_directories() {
        ...
        entries.insert(skill_root.join(".skillator-*"));
        if let Some(names) = expected_by_directory.get(directory.key().as_str()) {
            entries.extend(names.keys().map(|name| skill_root.join(name)));
        }
    }

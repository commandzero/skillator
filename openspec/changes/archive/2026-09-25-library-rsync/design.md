...
Create each session stage through an opened, non-following parent directory beneath home and record the new directory's device and inode from its open handle. Open an existing session lock through that parent without following the final component before retaining it. Validate that the stage remains the same physical directory within the participant's home before every rsync transfer, export, publication, and cleanup, including the initiating stage immediately before and after each remote push. For each transfer, open the local stage through directory handles under the observed participant home, compare its identity with the Begin response, and bind rsync to that inode as its working directory. The remote rsync entry point independently checks that the decoded stage belongs to its actual `HOME`, then opens and checks its identity before binding its server subprocess to that inode. Both sides pass only a relative entry name to rsync, so a pathname swap after validation cannot redirect the transfer. Encode the remote stage path in the wrapper command to preserve shell paths with spaces. Clean stage contents through the held parent handle. The private stage-validation request requires protocol 4; an older peer is rejected before writes.
...
Resolve existing expressions on the originating machine, prove home containment, then transfer the home-relative suffix. Read host, Library, User Scope, and synchronization history configuration through non-following directory handles and reject a linked or non-regular final file before it can enter a participant snapshot. Publish incoming Library registration conditionally through the same kind of held parent handle. Preserve matching receiving registrations and append missing registrations after successful preparation. Existing exclusion or overlap disagreements block the affected Location. Registration removal is deferred; it must not imply deleting a source tree.
...
Once a run has started publishing, a lost host does not trigger an unsafe attempt to erase successful independent work on every other host. Retained acknowledgements and recovery records make retry explicit. A concurrent change blocks the affected entry; it never receives an unconditional last-writer-wins overwrite. If usable peer acknowledgements disagree about a path's prior value, block that path as conflicted rather than choosing a baseline by participant order. If a selected peer has no matching baseline, do not infer a common baseline from only the other peers; independent present content from that first-contact peer remains a conflict. For each publication, walk the physical destination parent from an opened home directory using non-following directory handles. Create missing parent components, stage sibling content, rename, verify, and clean up relative to the held parent inode. Creation uses no-replace; replacement atomically exchanges the destination into a recoverable sibling; removal atomically moves it there. Verify the preserved sibling against the expected value after the rename and move entries back without discarding an intervening value on mismatch. Open copied file sources through a held, non-following parent handle as well. Publish synchronization history conditionally relative to an opened parent directory; verify the live state after exchanging it and preserve the prior version for recovery if another writer changes the live entry. A later ancestor symlink swap cannot redirect those writes outside the home; the logical path is rechecked before and after publication.

...
Once a run has started publishing, a lost host does not trigger an unsafe attempt to erase successful independent work on every other host. Retained acknowledgements and recovery records make retry explicit. A concurrent change blocks the affected entry; it never receives an unconditional last-writer-wins overwrite. For each publication, walk the physical destination parent from an opened home directory using non-following directory handles. Create missing parent components, stage sibling content, rename, verify, and clean up relative to the held parent inode. A later ancestor symlink swap cannot redirect those writes outside the home; the logical path is rechecked before and after publication.
